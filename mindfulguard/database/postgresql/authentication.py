...
class Authentication:
    ...
    async def sign_up(self,login:str,secret_string:str,reg_ip:str,confirm:bool,secret_code:str,backup_codes:list[int]):
        connection = None
        try:
            connection = await Connection().connect()
            value:int = await connection.fetchval('SELECT sign_up($1, $2, $3, $4, $5,$6)',login, secret_string, reg_ip, confirm, secret_code,backup_codes)#needs to be changed to False
    ...
    async def is_auth_admin(self,connection,token:str):
        is_auth:int = await connection.fetchval('''
