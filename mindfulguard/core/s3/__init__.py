...
from mindfulguard.core.s3.client import Client
...
class Object:
        ...
        try:
            for i in data:
                self.__s3_client.put_object(
                    self.__bucket_name,
                    object_name=prefix_object_name_+sha256s(str(i.filename))+_postfix_object_name,
                    data = io.BytesIO(await i.read()),
                    length= i.size,
                    metadata={"object_name":str(i.filename)}
                )
            return True
        except ValueError:
            return True
    ...
    def get_object(self, object_name: str):
        ...
        try:
            ...
            return (response.data,response.getheader('X-Amz-Meta-Object_name'))
        except S3Error:
            return None
        finally:
            if response:
