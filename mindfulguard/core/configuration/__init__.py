...
from mindfulguard.core.configuration.nestings.authentication import Totp, Public
from mindfulguard.core.configuration.nestings.safe import Lengths
import mindfulguard.core.configuration.nestings.item as item

class ServerConfiguration:
    __PATH_TO_SERVER_CONFIGURATION: str = 'mindfulguard/core/configuration/package/server_configuration.toml'
    def __init__(self):
...
class Item:
    def __init__(self,server_configuration:ServerConfiguration):
        self.__block:str = 'item'
        self.__config:ServerConfiguration = server_configuration
    def lengths(self):
        return item.Lengths(self.__block,self.__config)
    def categories(self):
        return item.Categories(self.__block,self.__config)
    def types(self):
        return item.Types(self.__block,self.__config)